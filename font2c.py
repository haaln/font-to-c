...
parser = argparse.ArgumentParser(
    prog="Font 2 C", description="Convert TTF fonts to C array for embedded devices"
)
...
def main():
    ...
    if int(args.size) % 8 != 0:
        ...
        exit(1)
    ClangGenerator(args.file, args.size)
...
class ClangGenerator(object):
    ...
    def generateCHeader(self):
        ...
        self.h_file.write(
            "extern const int TALLEST_CHAR_PIXELS;\n\n".format(
                self._getHeightOfTallestCharacter()
            )
        )
        self.h_file.write(
            "extern const uint8_t {}_pixels[];\n\n".format(self._getSafeName())
        )
        ...
        self.h_file.write("};\n\n")
        self.h_file.write(
            "extern const struct font_char {}_lookup[];\n".format(self._getSafeName())
        )
    ...
    def _generateLookupTable(self):
        self.c_file.write(
            "const struct font_char {}_lookup[] = {{\n".format(self._getSafeName())
        )
        self.c_file.write(
    ...
    def _getChar(self, char):
        ...
        top = self.pixelSize_int - self.face.glyph.bitmap_top
    ...
    def _getSafeName(self):
        return "{}_font".format(
            re.sub(r"[^\w\d]+", "_", self._getFriendlyName().lower())
        )

    def _getFriendlyName(self):
        return "{} {} {}".format(
            self.face.family_name, self.face.style_name, self.pixelSize_int
        )
